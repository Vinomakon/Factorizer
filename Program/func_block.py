...
class Function(pygame.sprite.Sprite):
    ...
    def decode_shape(self, shape1, shape2=None):
        shapes = []
        temp_layer = ""
        layers = []
        for letter in shape1:
            if letter == ":":
    ...
    def rotate_cw(self):
        shape = self.decode_shape(self.out1_data)
        self.out1_data = ""
        for layer in range(len(shape)):
            self.out1_data += shape[layer][6:8] + shape[layer][0:6]
            self.out1_data += ":"
    ...
    def rotate_full(self):
        ...
        self.out1_data = ""
        for layer in range(len(shape)):
            self.out1_data += shape[layer][4:8] + shape[layer][0:4]
            self.out1_data += ":"
        self.out1_data = self.out1_data[:-1]
    ...
    def cut(self):
        ...
        self.out1_data = ""
        for layer in range(len(shape)):
            self.out1_data += f"{shape[layer][0:2]}----{shape[layer][6:8]}"
            self.out1_data += ":"
            self.out2_data += f"--{shape[layer][2:6]}--"
            self.out2_data += ":"
        self.out1_data = self.out1_data[:-1]
